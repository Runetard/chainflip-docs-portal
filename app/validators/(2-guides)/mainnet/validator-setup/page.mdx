...
Some VPS providers enable an all-port open firewall by default. We recommend opening only the required ports, but if this is not the case your node will not be able to participate in Keygen / Signing ceremonies and as a result will never become active.
